...
const apiPaymentGateway = axios.create({
  baseURL:
    ...
    "https://api-sandbox.payment-gateway.dev/v1",
  headers: {
    "Content-Type": "application/json",
  },
});
...
export const paymentService = {
  createPayment: async (
    ...
    cardData: PaymentCardTokenResponse,
    acceptanseTems: AcceptanceTokensResponse,
  ...
  createCardToken: async (
    formData: PaymentFormData
  ): Promise<PaymentCardTokenResponse> => {
    try {
      ...
      if (!publicToken) {
        throw new Error(
          "No se encontró el token público para la transacción. Por favor, inicie el proceso de pago nuevamente."
        );
      }
      ...
      const response = await apiPaymentGateway.post<PaymentCardTokenResponse>(
        "/tokens/cards",
        cardData,
        {
          headers,
  ...
  createTransaction: async (
    ...
    try {
      const response = await api.post<{ paymentConfig: PaymentConfig }>(
        "/payments",
        formData
      );
  ...
  getAcceptanceTerms: async (): Promise<AcceptanceTokensResponse> => {
    try {
      const response = await api.get<AcceptanceTokensResponse>(
        "/acceptanceTokens"
      );
      return response.data;
    } catch (error) {
