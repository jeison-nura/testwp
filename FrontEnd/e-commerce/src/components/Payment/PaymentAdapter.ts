...
export const PaymentAdapter = {
  ...
  processPayment: async (
    ...
    productId?: string,
    quantity?: number,
    acceptanceTerms?: AcceptanceTokensResponse
    ...
    try {
      ...
      const paymentConfig = await paymentService.createTransaction(
        transactionData
      );
      transactionId = paymentConfig.reference;
      if (
        !paymentConfig ||
        !paymentConfig.publicToken ||
        !paymentConfig.paymentToken
      ) {
        throw new Error(
          "Respuesta de transacción inválida del servidor: faltan tokens necesarios"
        );
      }
      ...
      if (tokenResponse.status !== "CREATED" || !tokenResponse.data) {
        ...
        if (transactionId) {
          await paymentService.updateTransaction(transactionId, {
            status: "CANCELED",
            error_message:
              tokenResponse.error?.reason ||
              "Error en la tokenización de la tarjeta",
          });
        }
        ...
        return {
          ...
          error:
            tokenResponse.error?.reason ||
            "La transacción fue rechazada. Por favor, intente con otra tarjeta.",
        };
      }
      ...
      return {
        ...
        status: paymentResponse.status as
          | "PENDING"
          | "APPROVED"
          | "REJECTED"
          | "CANCELED",
        details: {
          ...
          expDate:
            tokenResponse.data.exp_month + "/" + tokenResponse.data.exp_year,
        },
      };
    } catch (error: any) {
      ...
      if (transactionId) {
        try {
          await paymentService.updateTransaction(transactionId, {
            status: "CANCELED",
            error_message:
              error.message || "Error durante el procesamiento del pago",
          });
        } catch (cancelError) {
          console.error(
            "Error al cancelar la transacción fallida:",
            cancelError
          );
        }
      }
